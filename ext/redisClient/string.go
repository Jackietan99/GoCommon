...
/**
* 向redis中写入一个key—val类型的字符串，如果存在则覆盖
* @key 	string 	参数的主键
* @val 	string 	需要写入缓存的值
* @Ex	int		超时时间（秒）
				参数为0时，永远不过期
...
* return	返回影响行数
*/
func (this *RedisPool) StringReWrite(key string, val string, EX int) {
...
/**
* 向redis中写入一个key—val类型的字符串，如果存在，则跳过
* @key 	string 	参数的主键
* @val 	string 	需要写入缓存的值
* @Ex	int		超时时间（秒）
				参数为0时，永远不过期
...
* return	返回影响行数
*/
func (this *RedisPool) StringWrite(key string, val string, EX int) int {
	var result = 0
...
* return	返回影响行数
*/
func (this *RedisPool) IntReplaceWrite(key string, val int, EX int) bool {
	var result = true
	var err error
	var res  interface{}
 ...
 *
 */
func (this *RedisPool) IntIncrease(key string) int {
	key = this.prefix + ":" + key
	var res int
 ...
 *
 */
func (this *RedisPool) IntDecrement(key string) int {
	key = this.prefix + ":" + key
	var res int
